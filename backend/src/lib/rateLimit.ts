...
export class PerUserUrlLimiter {
  ...
  isDuplicate(userId: string, canonicalUrl: string): boolean {
    const duplicate = this.hasDuplicate(userId, canonicalUrl);
    if (!duplicate) this.record(userId, canonicalUrl);
    return duplicate;
  }

  /**
   * Returns true when this (user, url) pair is still inside the TTL window.
   * Does not record the pair; use this before expensive work.
   */
  hasDuplicate(userId: string, canonicalUrl: string): boolean {
    ...
    if (expiresAt) this.store.delete(key);
    return false;
  }

  /** Records a successful roast generation or cached hit for this (user, url). */
