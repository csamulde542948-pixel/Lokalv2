...
/* ─── PostModal ─────────────────────────────────────────────────────────────── */
interface PostModalProps { postId: string; onClose: () => void; }
...
export function PostModal({ postId, onClose }: PostModalProps) {
  ...
  useEffect(() => { fetchPost({ variables: { id: postId } }); }, [postId]);
  ...
  return createPortal(
    <>
      ...
      <div className="fixed inset-0 z-[9001] flex items-center justify-center p-2 sm:p-4 pointer-events-none">
        <div
          className="relative w-full pointer-events-auto flex flex-col rounded-2xl overflow-hidden shadow-2xl border border-border bg-card"
          style={{ maxWidth: hasMedia ? 1280 : 560, maxHeight: "calc(100vh - 2rem)", height: "calc(100vh - 2rem)" }}
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 h-14 border-b flex-shrink-0 bg-card z-10">
            <h2 className="font-bold text-base truncate pr-4">
              {p?.author?.displayName ?? p?.author?.name ?? "Post"}'s post
            </h2>
            <button
              onClick={onClose}
            ...
            >
              ...
              {p && (
                <>
                  ...
                  <div className="flex-shrink-0 overflow-y-auto max-h-[40%]" style={{ scrollbarWidth: "none" }}>
                    ...
                    <div className="flex items-center gap-3 px-4 pt-4 pb-2">
                      ...
                      {!isOwnPost && (
                        <button
                          ...
                          className={`flex items-center gap-1 text-xs font-semibold px-2.5 py-1.5 rounded-md transition-colors flex-shrink-0 ${
                            isFollowing
